...
export function MobileNav({ current, isAdmin = false }: { current: Current; isAdmin?: boolean }) {
  ...
  const item = (href: string, label: string, active: boolean) => (
    <Link
      ...
      className={
        "block py-4 text-lg font-bold no-underline transition-colors " +
        (active ? "text-zinc-900 dark:text-zinc-50" : "text-zinc-600 dark:text-zinc-300")
      }
    >
      {label}
    </Link>
  ...
  const panel = (
    <div className="menu-slide-down fixed inset-x-0 bottom-0 top-16 z-40 flex flex-col bg-[var(--background)] px-5 py-4">
      ...
      <div className="mt-auto flex flex-col items-center gap-3">
        {isAdmin && (
          <Link
            ...
            className={
              "rounded-lg px-3 py-1.5 text-sm font-bold no-underline transition-colors " +
              (current === "admin" ? "text-zinc-900 dark:text-zinc-50" : "text-zinc-600 dark:text-zinc-300")
            }
          >
            Admin
          </Link>
